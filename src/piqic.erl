...
normalize(true, Acc, [H|T]) when ?IS_UPPER(H) ->
    normalize(true, [uncapitalize_c(H)|Acc], T);
normalize(false, Acc, [H|T]) when ?IS_UPPER(H) ->
    normalize(true, [uncapitalize_c(H), $_ |Acc], T);
normalize(_, Acc, [H|T]) ->
    normalize(false, [H|Acc], T).
...
uncapitalize_c(V) when ?IS_UPPER(V) -> V - $A + $a;
uncapitalize_c(V) -> V.
